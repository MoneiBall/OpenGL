...
bool CMyApp::Init()
{
    ...
    //////// 4 triangles for the base order: soldan saga
	////////------------------------------------------base
	add_triangle(
		glm::vec3(mx, 0, mz),
		glm::vec3(-mx, 0, mz),
	...
	m_mesh = ObjParser::parse("tree.obj");
	m_mesh->initBuffers();
...
//--------------------------------------------------------------------draw pyramid	
	...
	for (int i = 0; i < 10; i++) {
		double angle = 2 * M_PI / 10 * i + 2 * M_PI * t / 5000.0; //10 dene esya ucun yerlemse bucagi
		...
		//matWorld = glm::translate(glm::vec3(0, y, 0)); //set starting position for hexagonpyramid
		//transform pyramid positions (translate rotate)

		
			matWorld = glm::translate(glm::vec3(dirX + x, 20.0 - y, dirZ + z)) * glm::rotate<float>(M_PI, glm::vec3(1, 0, 0)) //basi asagi rotate
				*
		...
		//std::cout << tick_time << " " << " " << delta << " " << last_tick_time << std::endl;
		...
		// draw with VAO
		m_vb_pyram.On();
		m_vb_pyram.Draw(GL_TRIANGLES, 0, 30); //IB
		m_vb_pyram.Off();
	}
 //-------------------------------------------------------------------------------------------------------
	...
	matWorld = move * glm::translate<float>(glm::vec3(0, 2, 0)); //if asks for movement of object
	matWorldIT = glm::transpose(glm::inverse(matWorld));
	mvp = m_camera.GetViewProj() * matWorld;
